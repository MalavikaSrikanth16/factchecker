...
3. <a id="huggingface-auth"></a>[Required if using LLMs via Hugging Face Inference Client or Agentic RAG. See [Setting the Configuration](#setting-the-configuration)] Set up huggingface authentication :
...
4. <a id="izzyviz-install"></a>[Optional. See [Setting the Configuration](#setting-the-configuration)] To use the `izzyviz` feature to visualize LLM self-attention heat maps, you need to install IzzyViz :
